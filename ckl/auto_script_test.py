...
def feature_transform(data_all):
    feature_all = data_all.drop('plus_date', axis=1)
...
#    pull_data(sql_train, sql_test)
    # save daily csv
    training_filename = 'D:\\test\\training61.csv'
    testing_filename = 'D:\\test\\testing61.csv'
    daily_mysql_df = pd.read_csv(training_filename, low_memory = False)
    table_name = str(datetime.date.today())
    daily_mysql_df.to_csv('daily_train_' + table_name + '.csv', encoding='utf-8',index=False,index_label=True)
    
    daily_mysql_df_test = pd.read_csv(testing_filename, low_memory = False)
    table_name_test = str(datetime.date.today())
    daily_mysql_df_test.to_csv('daily_test_' + table_name_test + '.csv', encoding='utf-8',index=False,index_label=True)
    # Get data
...
#    label_train, label_test = get_labels(data_train, data_test)
    label_train = get_labels(data_train)
    label_test = get_labels(data_test)
    # Combine data
...
#    data_train, data_test = data_train_raw, data_test_raw
#
#    feature_train, feature_test = data_train.drop('plus_date', axis=1), data_test.drop('plus_date', axis=1)
#    label_train, label_test = data_train.plus_date, data_test.plus_date
#
...
if __name__ == '__main__':
    training_filename = 'D:\\test\\training61.csv'
    testing_filename = 'D:\\test\\huoke_20170929.csv'

#    label_dict = {'未投资': 0, '已投资': 1}

#    schedule_time = datetime.datetime(2017, 10, 13, 16, 09)
