...
def pull_data(sql_train, sql_test):
    ...
    for elt in enumerate(parsedsql_train):
        ...
        filename = 'yyh_training' + str(elt[0]) +'.csv' 
    ...
    for elt in enumerate(parsedsql_test):
        ...
        print(result.head())
        print("pulling data is done --- %s seconds ---" % round((time.time() - start_time),2))
        filename = 'yyh_testing' + str(elt[0]) +'.csv' 
        result.to_csv(filename, encoding='utf-8',index=False,index_label=True)
        print("saving data is done --- %s seconds ---" % round((time.time() - start_time),2))
    hive_client.close()
...
def main():
    ...
    sql_test = sql_test.replace('final_train_baiyang_14', 'final_test_baiyang').replace('final_train_baiyang_14', 'final_test_baiyang')

    # Pull data from Hive

    pull_data(sql_train, sql_test)
    
    train_filename = 'yyh_training61.csv'
    test_filename = 'yyh_testing61.csv'
    mysql_df = pd.read_csv(train_filename, low_memory = False)
    today = datetime.date.today()
    yesterday = today -datetime.timedelta(days=1)
    table_name = str(yesterday)
    mysql_df.to_csv('yyh_oot_train_' + table_name + '.csv', encoding='utf-8',index=False,index_label=True)
    
    mysql_df_test = pd.read_csv(test_filename, low_memory = False)
    table_name_test = str(yesterday)
    mysql_df_test.to_csv('yyh_oot_test_' + table_name_test + '.csv', encoding='utf-8',index=False,index_label=True)

if __name__ == '__main__':

    schedule_time = datetime.datetime(2017, 10, 18, 10,50 )
