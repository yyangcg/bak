...
def scheduler(time):
    ...
    scheduler.add_job(main, 'interval', seconds=86400, next_run_time=time)
    scheduler.start()
...
drop table if exists qianjin_test.test07191_final_new_track_14;
...
select * from qianjin_develop.test_finaltrack_new where 0=1;
...
INSERT overwrite table  qianjin_test.test_yimei07192_track_3
SELECT DISTINCT a.* ,
                CASE
...
INSERT overwrite table  qianjin_test.test_waihu0719_track_4
SELECT user_id,
...
INSERT overwrite table  qianjin_test.test07191_final_new_track_14
SELECT a.user_id,
       a.mobile,
       a.isinvested,
       a.user_age,
...
select * from qianjin_test.test07191_final_new_track_14
'''
sql_test = sql_train.replace('statistic_dayx', 'statistic_dayb').replace('statistic_dayy', 'statistic_daye')
...
def pull_data():
    global statistic_dayb
    global statistic_daye
    ...
    for elt in enumerate(parsedsql_train):
        sqltmp = elt[1].replace(';','').replace('statistic_dayx',statistic_dayx).replace('statistic_dayy',statistic_dayy).encode('utf8')
        print(sqltmp)
        result = hive_client.query(sqltmp)
        print(result.head())
        print("pulling data is done --- %s seconds ---" % round((time.time() - start_time),2))
        filename = 'training_1' + str(elt[0]) +'.csv' 
    ...
    for elt in enumerate(parsedsql_test):
        sqltmp = elt[1].replace(';','').replace('statistic_dayb',statistic_dayb).replace('statistic_daye',statistic_daye).encode('utf8')
        print sqltmp
        result = hive_client.query(sqltmp)
        print result.head()
        print "pulling data is done --- %s seconds ---" % round((time.time() - start_time),2)
...
def main():
    ...
    # Connect to server
    conn = mdb.connect(host='192.168.143.201', user='phdc_calvinku', 
                       password='phdc_calvinku.XLPfZrvjMlsFqSqfxOSBcDc1EOGp5Xy8', port=3306, db="policy_jtyy", charset="utf8")
    # Create cursor
    cur = conn.cursor()
    ...
    cur.execute("DROP TABLE IF EXISTS ck_user_id_score")
...
if __name__ == '__main__':	
    statistic_dayx, statistic_dayy = '''''' + time_func(38) + '''''', '''''' + time_func(36) + ''''''
    statistic_dayb, statistic_daye = '''''' + time_func(0) + '''''', '''''' + time_func(0) + ''''''
    schedule_time = datetime.datetime(2017, 9, 20, 13, 32)
