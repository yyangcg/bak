...
def scheduler(schedule_time):
    ...
    # scheduler.add_job(main, 'interval', seconds=7200, next_run_time=time)
    scheduler.start()
...
INSERT overwrite table  qianjin_test.test_yimei07192_track
SELECT DISTINCT a.* ,
                CASE
...
INSERT overwrite table  qianjin_test.test_waihu0719_track
SELECT user_id,
...
select * from qianjin_test.final_train_baiyang
'''
...
def add_new_record_to_sql_table(conn, sql_cursor, data_tuple, db='policy_jtyy', table='ck_user_id_score'):
...
def add_new_record_to_sql_table_lift(conn, sql_cursor, data_tuple, db='policy_jtyy', table='ck_lift_score'):
...
def lift_main():
    ...
    update_db_lift(user_lift_score, 'ck_lift_score')

if __name__ == '__main__':	
    statistic_dayx, statistic_dayy = '"' + time_func(38) + '"', '"' + time_func(8) + '"'
    statistic_dayb, statistic_daye = '"' + time_func(1) + '"', '"' + time_func(1) + '"'
    # statistic_dayx, statistic_dayy = '''"2017-08-01"''', '''"2017-08-02"'''
    # statistic_dayb, statistic_daye = '''"2017-09-01"''', '''"2017-09-01"'''
    # schedule_time = datetime.datetime(2017, 9, 25, 07, 30)
