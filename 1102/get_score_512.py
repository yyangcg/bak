...
def calc_continus_ks(data, prediction="prediction", label="isinvested"):
    """
...
clf=RandomForestClassifier(n_estimators=512,max_depth=10,random_state=50,oob_score=True,min_samples_leaf=20, n_jobs = -1, min_samples_split=20)
...
result_train_ks = result_train.sort_values(by='prediction', ascending=False)
ks_y_train=calc_continus_ks(result_train_ks,prediction="prediction")
result_test_ks = result_test.sort_values(by='prediction', ascending=False)
ks_y_test=calc_continus_ks(result_test_ks,prediction="prediction")
print(max(ks_y_train))
print(max(ks_y_test))

Data.to_csv('06_09_call_score_rf_512.csv',index=False, index_label=False)
print("Training 512rf {0} seconds.".format(time.time() - start_time))